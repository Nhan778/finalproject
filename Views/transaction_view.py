        ...
        with col_edit:
            with col_edit:
                ...
                with st.popover("✏️ Edit"): #, key=f"pop_edit_{item['_id']}"
                    ...
                    # Load transaction
                    transaction = model.get_transaction_by_id(str(item['_id']))
                    ...
                    if not transaction:
                        ...
                        new_amount = st.number_input(
                            ...
                            key=f"edit_amt_{item['_id']}"
                        )

                        new_date = st.date_input(
                            "Transaction Date",
                            trans_date,
                            key=f"edit_date_{item['_id']}"
                        )

                        new_description = st.text_area(
                            "Description",
                            transaction.get("description", ""),
                            key=f"edit_desc_{item['_id']}"
                        )

                        save = st.button("💾 Save", key=f"save_edit_{item['_id']}")

                        if save:
                            model.update_transaction(str(item['_id']), {
                                "transaction_type": new_type,
                                "category": new_category,
                                "amount": new_amount,
                                "transaction_date": datetime.combine(new_date, datetime.now().time()),
                        ...
                        cancel = st.button("❌ Cancel", key=f"cancel_edit_{item['_id']}")
        with col_delete:
...
def render_transaction(transaction_model, category_model):
