...
def format_currency(amount):
    ...
    return f"${amount:,.2f}" # round to 2 decimal, and add "," 

def get_date_range_options():
    """Get predefinded date range options"""
    now = datetime.now()
    today = now.date()
    ...
    return{
        ...
        "Last 3 month":(today - timedelta(days=90), today),
        "Last 6 month":(today - timedelta(days=180), today),
...
def validate_amount(amount):
    ...
    try:
        value = float(amount)
        if value <0:
            return False, "Amount must greater than zero"
        return True, value
    except ValueError:
...
# cách dùng | type | type | => Dùng import Unio rồi chỗ dữ liệu chuyển đổi thành Unio[date, datetime]
